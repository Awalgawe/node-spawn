...
describe("spawn function", () => {
  ...
  test("should handle stderr output", async () => {
    const result = await spawn("node", ["-e", "console.error('test error'); process.exit(0)"]);
    
    assert.strictEqual(result.code, 0);
    assert.strictEqual(result.stderr.trim(), "test error");
  });
  ...
  test("should process stderr with callback", async () => {
    ...
    const result = await spawn("node", ["-e", "console.error('error message')"], {
  ...
  test("should process both stdout and stderr with callbacks", async () => {
    ...
    const result = await spawn("node", ["-e", "console.log('stdout'); console.error('stderr')"], {
      onStdOut: (chunk, abort) => {
  ...
  test("should abort command using abort function from callback", async () => {
    ...
    try {
      await spawn("node", ["-e", "console.log('start'); setInterval(() => console.log('tick'), 100);"], {
        onStdOut: (chunk, abort) => {
  ...
  test("should collect stderr output even when command fails", async () => {
    try {
      await spawn("node", ["-e", "console.error('error before exit'); process.exit(1)"]);
      assert.fail("Expected command to fail");
    } catch (err) {
      assert.ok(err instanceof Error);
      assert.ok(err.cause && typeof err.cause === 'object');
  ...
  test("should still call callbacks even when command fails", async () => {
    ...
    try {
      await spawn("node", ["-e", "console.error('error message'); process.exit(1)"], {
        onStdErr: (chunk, abort) => {
          stderrChunks.push(chunk.toString());
        }
      });
